...
# is there a better way to downsample than just not taking the ones after there is enough data
# somehow randomly pick the texts to save?
# => I could try shuffling the data before I downsample it?

# downsample EVERY category (or almost every category?)

# we talked about training data but what about the dev and test?
# do we just split the data to those sets after translation

# the english lines with empty labels are now removed but there is the possibility to change them to other
...
def formatEN_FIN(data):
    ...
    for i in range(len(data)):
        ...
        # if the first column is empty we put it to other (for english data)
        # we also could just remove those lines? because there is really no value to that?
        # and it only is in the english data
        if data[i][0] == "":
            #data[i][0] = "other"
            indeces_delete.append(i)
            #print(data[i], i)
